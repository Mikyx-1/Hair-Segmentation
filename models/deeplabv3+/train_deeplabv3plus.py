...
def train(epochs):
   ...
   for epoch in range(1, epochs+1):
    ...
    val_loss, meanIOU = evaluate()
    train_losses.append(train_loss)
    val_losses.append(val_loss)
    val_IOU.append(meanIOU)
    ...
    if meanIOU > maxIOU:
       maxIOU = meanIOU
       torch.save(model.state_dict(), "maxIOU.pt")
        
    torch.save(model.state_dict(), "last.pt")
    train_hist = pd.DataFrame({"Epoch": [i for i in range(1, epoch+1)], "train_loss": train_losses, "val_loss": val_losses, "val_IOU": val_IOU})
    train_hist.to_csv("./train_history.csv")
    print(f"Epoch: {epoch}/{epochs} Train Loss: {train_loss} Val Loss: {val_loss} MeanIoU: {meanIOU}")
